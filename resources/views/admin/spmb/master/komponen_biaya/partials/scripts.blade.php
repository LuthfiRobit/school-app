        ...
        return {
            ...
            initDataTable() {
                ...
                this.table = $('#feeTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('admin.spmb.master.komponen-biaya.data') }}",
                    columns: [
            ...
            async bulkStatus(status) {
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.post("{{ route('admin.spmb.master.komponen-biaya.bulk-status') }}", {
            ...
            async submitForm() {
                ...
                const url = this.isEdit 
                    ? `{{ route('admin.spmb.master.komponen-biaya.update', ':id') }}`.replace(':id', this.editId)
                    : `{{ route('admin.spmb.master.komponen-biaya.store') }}`;
            ...
            async editData(id) {
                try {
                    const response = await axios.get(`{{ route('admin.spmb.master.komponen-biaya.show', ':id') }}`.replace(':id', id));
                    this.formData = {
            ...
            async deleteData(id) {
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.delete(`{{ route('admin.spmb.master.komponen-biaya.destroy', ':id') }}`.replace(':id', id));
            ...
            async toggleStatus(id) {
                try {
                    const response = await axios.post(`{{ route('admin.spmb.master.komponen-biaya.toggle-status', ':id') }}`.replace(':id', id));
                    toastr.success(response.data.message);
                    this.refreshTable();
                } catch (error) {
