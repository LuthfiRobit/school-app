        ...
        return {
            ...
            initDataTable() {
                ...
                this.table = $('#trackTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('admin.spmb.master.jalur.data') }}",
                    columns: [
                        { data: 'checkbox', name: 'checkbox', orderable: false, searchable: false },
                        { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                        { data: 'action', name: 'action', orderable: false, searchable: false },
                        { data: 'name', name: 'name' },
                        { data: 'description', name: 'description', render: (data) => data ? (data.length > 50 ? data.substring(0, 50) + '...' : data) : '-' },
            ...
            setupEventListeners() {
                ...
                $table.on('change', '.toggle-status', function() {
                    ...
                    $(this).prop('checked', !isChecked); // Revert temporarily
            ...
            async bulkStatus(status) {
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.post("{{ route('admin.spmb.master.jalur.bulk-status') }}", {
            ...
            confirmToggle(id) {
                Swal.fire({
                    ...
                    text: "Apakah Anda yakin ingin mengubah status aktif jalur ini?",
                    icon: 'question',
                    showCancelButton: true,
            ...
            async submitForm() {
                ...
                const url = this.isEdit 
                    ? `{{ route('admin.spmb.master.jalur.update', ':id') }}`.replace(':id', this.editId)
                    : `{{ route('admin.spmb.master.jalur.store') }}`;
            ...
            async editData(id) {
                try {
                    const response = await axios.get(`{{ route('admin.spmb.master.jalur.show', ':id') }}`.replace(':id', id));
                    this.formData = {
            ...
            async deleteData(id) {
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.delete(`{{ route('admin.spmb.master.jalur.destroy', ':id') }}`.replace(':id', id));
            ...
            async toggleStatus(id) {
                try {
                    const response = await axios.post(`{{ route('admin.spmb.master.jalur.toggle-status', ':id') }}`.replace(':id', id));
